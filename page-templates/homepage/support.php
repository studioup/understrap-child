<div class="layout">
  <div class="layout__col1 border-bottom">
  </div>
  <div class="layout__col2 container border-bottom">
    <div class="row no-gutters">
      <div class="col-sm-10 border-right">
        <div class="row">
          <div class="col-sm-11">
            <div class="support mt-7 mb-7 pl-4 pt-6 pb-7">
              <div class="support__corner support__corner--lt">
                <img src="<?php echo get_stylesheet_directory_uri() . '/img/corner.svg'; ?>" />
              </div>
              <div class="support__corner support__corner--rb">
                <img src="<?php echo get_stylesheet_directory_uri() . '/img/corner.svg'; ?>" />
              </div>
              <div class="support__grid">
                <img src="<?php echo get_stylesheet_directory_uri() . '/img/grid.svg'; ?>" />
              </div>
              <div class="support__anim anim_02" id="anim02"></div>
              <h2 class="h1 text-primary">We’re never on
      energy-saving mode</h2>
              <h3 class="mt-3">We provide certified Ablerex<br>assistance all around Europe.</h3>
              <a href="#" class="btn btn-secondary mt-3">Get in touch</a>
            </div>
          </div>
        </div>

      </div>
      <div class="col-sm-2 "><div class="pt-6"></div></div>
    </div>
  </div>
  <div class="layout__col3">
  </div>
</div>
<div class="layout">
  <div class="layout__col1 ">
    <div class="border-right pt-4 background-dotted-red"></div>
  </div>
  <div class="layout__col2 container border-right">
    <div class="row no-gutters">
      <div class="col-sm-2 offset-sm-10">
        <div class="pt-4 background-dotted-red border-left border-bottom"></div>
      </div>
    </div>
  </div>
  <div class="layout__col3 border-bottom">
  </div>
</div>
<div class="layout">
  <div class="layout__col1 border-right border-bottom background-dotted-red">

  </div>
  <div class="layout__col2 container border-bottom">
    <div class="row">
      <div class="col-sm-9 offset-sm-2">
        <div class="support2 mt-5 mb-5">
          <div class="support2__anim anim_03" id="anim03"></div>
          <div class="row">
            <div class="col-sm-6 offset-sm-3">
              <p class="font-big pt-7 pb-7 paragraph">Ut enim ad minim veniam, quis nostrud <span>exercitation</span> ullamco nisi ut. Enim ipsum dolor sit amet, adipiscing elit, sed do <span>eiusmod tempor</span> incididunt ut labore et magna aliqua.</p>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
  <div class="layout__col3">
  </div>
</div>
